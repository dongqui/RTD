...
export class ComposerUI {
  ...
  async loadSpineData() {
    try {
      const response = await fetch('../public/assets/spine/Fantasy Character.json');
  ...
  updateOutput() {
    ...
    const formattedParts = this.selectedParts.map((p) => `    "${p}"`).join(',\n');

    tsOutput.innerHTML = `<code>private static readonly SKIN_KEYS = [\n${formattedParts}\n];</code>`;
    jsonOutput.innerHTML = `<code>${JSON.stringify(this.selectedParts, null, 2)}</code>`;
  }
  ...
  setupEventListeners() {
    document.getElementById('animation-select')!.addEventListener('change', (e) => {
      ...
      if (this.spineObject && this.spineObject.animationState) {
        this.spineObject.animationState.setAnimation(0, this.currentAnimation, true);
      }
    });
    ...
    document.getElementById('reset-btn')!.addEventListener('click', () => {
      this.selectedParts = [];
      document.querySelectorAll('.part-item input[type="checkbox"]').forEach((cb) => {
  ...
  applyPreset(name: string) {
    ...
    this.selectedParts.forEach((skinName) => {
      const checkbox = document.getElementById(`part-${skinName}`) as HTMLInputElement;
      if (checkbox) {
