...
export class FrostTower extends BaseTower {
  ...
  protected setupAnimations(): void {
    const skin = new Skin("frost_tower");
    const fullskin = this.spineObject.skeleton.data.findSkin("full_skins_f");
    if (fullskin) {
      skin.addSkin(fullskin);
    }
    const boots = this.spineObject.skeleton.data.findSkin("boots/boots_f_10");
    if (boots) {
      skin.addSkin(boots);
    }
    const left = this.spineObject.skeleton.data.findSkin("gear_left/gear_left_f_3");
    if (left) {
      skin.addSkin(left);
    }
    const right = this.spineObject.skeleton.data.findSkin("gear_right/gear_right_f_3");
    if (right) {
      skin.addSkin(right);
    }
  ...
  protected performAttack(target: Phaser.GameObjects.GameObject): void {
    if (target instanceof BaseMonster) {
      ...
      console.log(
        `Frost Tower Lv.${this.level} at (${this.gridX}, ${this.gridY}) attacks target for ${this.damage} damage`
      );
    }
  }
